...
- 解析归属与边界：
  ...
  - app/扩展新增 `parseMicEvents(sourceText, documentConfig, dramarkAst)`，输出 `MicEvent[]` + `MicDiagnostic[]`。
- 指令输入形态（前端两种形式都识别）：
  - 行内 tech cue 形式：基于 `<<...>>` 内容做关键词匹配（关键词可配置）。
  - Markdown code block 形式：约定 fenced block（例如 ` ```dramark-tech `）内逐行指令，便于批量技术排练稿。
- 配置模型（frontmatter，允许缺省与自定义）：
  - `tech.micDirectives`：关键词字典（如 `switch`, `to`, `from`, `for`，以及本地化中文同义词）。
...
## Test Plan
- 识别层单测：
  - 中文/英文关键词与自定义关键词均能识别为 `MicEvent`。
  - 两种输入形态（inline cue / fenced tech block）均可解析。
